...
/**
 * ============================================================
 *  VALISTOQUE - Cadastro de novo usuário
 *  Recebe POST: nome, email, cpf, senha, confirmar_senha, perfil
 * ============================================================
 */
require_once __DIR__ . '/../includes/config.php';

if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    redirecionar('../../frontend/login.html');
}
...
// ---------- Validações ----------
if ($nome === '' || strlen($nome) < 3) alertaJs('Informe um nome válido (mínimo 3 caracteres).');
if (!validarEmail($email))             alertaJs('E-mail inválido!');
if (!validarCpf($cpf))                 alertaJs('CPF inválido!');
if (strlen($senha) < 6)                alertaJs('A senha deve ter no mínimo 6 caracteres.');
...
try {
    ...
    // Se for funcionário criado por admin logado, registra criador
